...
#MIles - KIlometers   
def m_k(a:int) -> int:
"""Конвертирует Мили в Киллометры"""
    s = a * 1.609
    return f"{a} Миль = {round(s,2)} Километров \n"

#KIlometers - MIles
def k_m(a:int) -> int:
"""Конвертирует Киллометры в Мили"""
    s = a / 1.609
...
#Funts - Kilograms
def f_k(a:int) -> int:
    """Конвертирует Фунты в Киллограмы"""
    s = a * 0.453592
...
#Kilograms - Funts
def k_f(a:int) -> int:
"""Конвертирует Киллограмы в Фунты"""
    s = a * 2.20462
    return f"{a} Килограм = {round(s, 2)} Фунтов \n"

#Unci - Gram
def y_g(a:int) -> int:
"""Конвертирует Унции в Граммы"""
    s = a * 28.3495
...
#Pinta - Litres
def p_l(a:int) -> int:
"""Конвертирует Пинты в Литры"""
    s = a * 0.568261
    return f"{a} Пинты = {round(s, 2)} Литров \n"

#Litres - Pinta
def l_p(a:int) -> int:
"""Конвертирует Литры в Пинты"""
    s = a * 1.75975
    return f"{a} Литров = {round(s, 2)} Пинты \n"

def main():
    """Это конвертер
При запуске этой функции вы увидиите приветствие и дальнейшую инструкцию
по управлению им. Вводите число и следуйте дальнейшим инструкциям, точность
конвертера равна 99.99999999999999999999999%, проверял лично. Чтобы остановить
выполнение скрипта нажмите 0 , а можете просто выдернуть шнур из БП.
    """
    while True:
